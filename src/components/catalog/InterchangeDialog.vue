  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-[440px]">
      <DialogHeader>
        ...
        <DialogDescription class="text-[12px] text-[#888]">Add an OE or competitor cross-reference.</DialogDescription>
      </DialogHeader>

      <div class="grid gap-4 py-2">
        <div>
          <label class="text-[11px] text-[#888] uppercase tracking-wider mb-1 block">Type</label>
          <Select v-model="form.type">
            <SelectTrigger class="h-8 text-[13px] border-[#ddd] w-40">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="text-[11px] text-[#888] uppercase tracking-wider mb-1 block">Brand Name</label>
            <Input v-model="form.brandName" class="h-8 text-[13px] border-[#ddd]" />
          </div>
          <div>
        ...
        <div>
          ...
          <Input v-model="form.notes" placeholder="Optional notes..." class="h-8 text-[13px] border-[#ddd]" />
        </div>
      </div>
