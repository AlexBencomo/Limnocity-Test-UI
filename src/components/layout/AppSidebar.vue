...
<template>
  <aside class="flex flex-col w-[200px] shrink-0 bg-[#1a1a2e] text-[#b0b0b0] overflow-hidden">
    <!-- Logo -->
    ...
    <nav class="flex-1 overflow-y-auto py-2">
      ...
      <button
        ...
        :class="route.name === 'dashboard'
          ? 'bg-[#3bbfa0] text-white font-medium'
          : 'hover:bg-white/[0.06]'"
        ...
        <div v-show="catalogOpen" class="mt-0.5">
          <button
            ...
            :class="isTabActive(tab.name)
              ? 'bg-[#3bbfa0] text-white font-medium'
              : 'hover:bg-white/[0.06]'"
            @click="handleTabClick(tab.name); router.push('/catalog')"
          >
      ...
      <div class="mt-3">
        ...
        <div v-show="settingsOpen" class="mt-0.5">
          <button
            class="flex items-center gap-2.5 w-[calc(100%-12px)] mx-1.5 px-3 py-1.5 rounded text-[13px] transition-colors"
            :class="route.name === 'brand-settings'
              ? 'bg-[#3bbfa0] text-white font-medium'
              : 'hover:bg-white/[0.06]'"
            @click="router.push('/brand-settings')"
          >
